...
module.exports.FollowMember = async (req, res) => {
    ...
    if (username === followMember) return res.status(400).json('You cannot follow yourself');
    ...
    try {
        ...
        if (findUser[0].following.includes(followMember)) {
            const unfollowings = await db.Users.updateOne({username}, {$pull: {following: followMember}});
