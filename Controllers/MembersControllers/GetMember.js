...
module.exports.GetMember = async (req, res) => {
  ...
  try {
    ...
    if (follows.includes(findUserUsername)) {
      ...
      if (userPosts.length === 0)
        return res.status(200).json({
          ...
          loggedInUser: loggedInUser,
          posts: userPosts,
          postMessage: `${findUserUsername} has no posts`,
        });
      return res
        .status(200)
        .json({ user: findUser, loggedInUser: loggedInUser, posts: userPosts });
    }
    ...
    res.status(200).json({
      ...
      loggedInUser: loggedInUser,
      posts: userPosts,
      followMessage: `You need to follow ${findUserUsername} to see their posts.`,
    });
    return;
  } catch (error) {
