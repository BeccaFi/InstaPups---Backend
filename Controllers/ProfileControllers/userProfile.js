...
exports.userProfile = async function userProfile (req, res) {
    ...
    const clickedUser = await db.Users.findOne({_id: new ObjectId(id)}, {projection: {password: 0}});
    const clickedUserUsername = clickedUser.username; //To get the username of whoevers' profile we're viewing, not our own username from req.user.
    ...
    try{
        ...
        if(follows.includes(clickedUserUsername)) {
            const userPosts = await db.Posts.find({username: clickedUserUsername}).sort({datePosted: -1}).toArray();
            return res.status(200).json({user: clickedUser, posts: userPosts});
        }
        return res.status(200).json({user: clickedUser, message: `You need to follow ${clickedUserUsername} to see their posts.`});
