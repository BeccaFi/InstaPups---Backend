...
describe('Check success when user submits a new create post form', () => {
    ...
    it('POST to /posts/create should return 201 when successfully added to database', async () => {
        ...
        const res = await request(server).post('/posts/create').send({datePosted: '2023-04-13 15:03', text: 'Today I ate a dust ball before realizing it was a dust ball. Now a guilty pleasure.', photos: [`http://dustballpics.com/?dustballnr=4`]}).set('Cookie', cookies);;
